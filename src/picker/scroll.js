...
const _rowHeight=window.innerWidth/100*8 //根据vw算px
/**
 * set transition
 * @param $target
 * @param time
...
const setTranslate = ($target, diff) => {
    diff=(''+diff).indexOf('vw')>0?diff:diff+'px';    
    return $target.css({
        '-webkit-transform': `translate3d(0, ${diff}, 0)`,
...
const getMax = (offset, rowHeight) => {
    console.log('max:', offset * rowHeight);
...
$.fn.scroll = function (options) {
    const defaults = $.extend({
        ...
        rowHeight: _rowHeight,                              // 列表每一行的高度
        onChange: $.noop,                           // onChange回调
        temp: null,                                 // translate的缓存
        bodyHeight: 7 * _rowHeight                         // picker的高度，用于辅助点击滚动的计算
    }, options);
    ...
    const stop = (diff) => {
        ...
        // 如果是 disabled 的就跳过
        let index = defaults.offset - parseInt(translate / defaults.rowHeight);
        ...
        setTranslate($scrollable, translate/(window.innerWidth/100)+'vw');
